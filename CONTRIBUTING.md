# Contributing to OpenCarrier

Thank you for your interest in contributing to OpenCarrier. This guide covers everything you need to get started, from setting up your development environment to submitting pull requests.
...
```bash
cargo build --profile release-fast -p opencarrier-cli
```
...
```bash
cargo test -p opencarrier-kernel
cargo test -p opencarrier-runtime
cargo test -p opencarrier-memory
```
...
- **Naming**:
  - Types: `PascalCase` (e.g., `OpenCarrierKernel`, `AgentManifest`)
  - Functions/methods: `snake_case`
  - Constants: `SCREAMING_SNAKE_CASE`
  - Crate names: `opencarrier-{name}` (kebab-case)
...
OpenCarrier is organized as a Cargo workspace with 14 crates:

| Crate | Role |
|-------|------|
| `opencarrier-types` | Shared type definitions, taint tracking, manifest signing (Ed25519), model catalog, MCP/A2A config types |
| `opencarrier-memory` | SQLite-backed memory substrate with vector embeddings, usage tracking, canonical sessions, JSONL mirroring |
| `opencarrier-runtime` | Agent loop, 3 LLM drivers (Anthropic/Gemini/OpenAI-compat), 38 built-in tools, WASM sandbox, MCP client/server, A2A protocol |
| `opencarrier-hands` | Hands system (curated autonomous capability packages), 7 bundled hands |
| `opencarrier-extensions` | Integration registry (25 bundled MCP templates), AES-256-GCM credential vault, OAuth2 PKCE |
| `opencarrier-kernel` | Assembles all subsystems: workflow engine, RBAC auth, heartbeat monitor, cron scheduler, config hot-reload |
| `opencarrier-api` | REST/WS/SSE API (Axum 0.8), 76 endpoints, 14-page SPA dashboard, OpenAI-compatible `/v1/chat/completions` |
| `opencarrier-channels` | 40 channel adapters (Telegram, Discord, Slack, WhatsApp, and 36 more), formatter, rate limiter |
| `opencarrier-wire` | OFP (OpenCarrier Protocol): TCP P2P networking with HMAC-SHA256 mutual authentication |
| `opencarrier-cli` | Clap CLI with daemon auto-detect (HTTP mode vs. in-process fallback), MCP server |
| `opencarrier-migrate` | Migration engine for importing from OpenClaw (and future frameworks) |
| `opencarrier-skills` | Skill system: 60 bundled skills, FangHub marketplace, OpenClaw compatibility, prompt injection scanning |
| `opencarrier-desktop` | Tauri 2.0 native desktop app (WebView + system tray + single-instance + notifications) |
| `xtask` | Build automation tasks |

### Key Architectural Patterns

- **`KernelHandle` trait**: Defined in `opencarrier-runtime`, implemented on `OpenCarrierKernel` in `opencarrier-kernel`. This avoids circular crate dependencies while enabling inter-agent tools.
- **Shared memory**: A fixed UUID (`AgentId(Uuid::from_bytes([0..0, 0x01]))`) provides a cross-agent KV namespace.
- **Daemon detection**: The CLI checks `~/.opencarrier/daemon.json` and pings the health endpoint. If a daemon is running, commands use HTTP; otherwise, they boot an in-process kernel.
...
```bash
opencarrier agent spawn agents/my-agent/agent.toml
```
...
Channel adapters live in `crates/opencarrier-channels/src/`. Each adapter implements the `ChannelAdapter` trait.

### Steps

1. Create a new file: `crates/opencarrier-channels/src/myplatform.rs`
...
3. Register the module in `crates/opencarrier-channels/src/lib.rs`:

```rust
pub mod myplatform;
```

4. Wire it up in the channel bridge (`crates/opencarrier-api/src/channel_bridge.rs`) so the daemon starts it alongside other adapters.

5. Add configuration support in `opencarrier-types` config structs (add a `[channels.myplatform]` section).

6. Add CLI setup wizard instructions in `crates/opencarrier-cli/src/main.rs` under `cmd_channel_setup`.
...
Built-in tools are defined in `crates/opencarrier-runtime/src/tool_runner.rs`.
...
- Open a [GitHub Discussion](https://github.com/RightNow-AI/opencarrier/discussions) for questions.
- Open a [GitHub Issue](https://github.com/RightNow-AI/opencarrier/issues) for bugs or feature requests.

...
impl ChannelAdapter for SmartBotChannel {
    ...
    fn send(&self, tenant_id: &str, user_id: &str, text: &str) -> Result<(), PluginError> {
        ...
        // Use a dedicated runtime since cdylib cannot use host's tokio runtime
        let rt = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .build()
            .map_err(|e| PluginError::channel(format!("Runtime creation failed: {e}")))?;

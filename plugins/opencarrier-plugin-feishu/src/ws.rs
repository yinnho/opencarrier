...
use opencarrier_plugin_sdk::{MessageSender, PluginContent, PluginMessage};
...
impl FeishuWsClient {
    ...
    pub async fn run(&self, sender: &MessageSender) {
    ...
    async fn connect_and_listen(&self, sender: &MessageSender) -> Result<(), String> {
        // Get token
    ...
    fn handle_frame(&self, text: &str, sender: &MessageSender) {

...
pub async fn get_bot_qrcode(http: &Client) -> Result<QrCodeResponse, String> {
    let url = format!("{ILINK_API_BASE}/ilink/bot/get_bot_qrcode?bot_type={BOT_TYPE}");
    let resp = http
        .get(&url)
        .send()
        .await
...
pub async fn get_qrcode_status(http: &Client, qrcode: &str) -> Result<QrCodeStatusResponse, String> {
    let url = format!(
        "{ILINK_API_BASE}/ilink/bot/get_qrcode_status?qrcode={}",

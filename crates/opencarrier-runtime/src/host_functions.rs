...
use opencarrier_types::capability::{capability_matches, Capability};
use serde_json::json;
...
fn host_agent_send(state: &GuestState, params: &serde_json::Value) -> serde_json::Value {
    ...
    match state
        .tokio_handle
        .block_on(kernel.send_to_agent(target, message, None, None, None, None))
    {
...
fn host_agent_spawn(state: &GuestState, params: &serde_json::Value) -> serde_json::Value {
    ...
    let tenant_id = kernel
        .get_agent_tenant_id(&state.agent_id)
        .unwrap_or_default();

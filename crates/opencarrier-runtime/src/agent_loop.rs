...
pub struct AgentLoopResult {
    ...
    pub directives: opencarrier_types::message::ReplyDirectives,
}

/// Run the agent execution loop for a single user message.
///
/// This is the core of OpenCarrier: it loads session context, recalls memories,
...
pub async fn run_agent_loop(
    ...
    if let Some(hook_reg) = hooks {
        let ctx = crate::hooks::HookContext {
            ...
            event: opencarrier_types::agent::HookEvent::BeforePromptBuild,
            ...
            StopReason::EndTurn | StopReason::StopSequence => {
                ...
                if let Some(hook_reg) = hooks {
                    let ctx = crate::hooks::HookContext {
                        agent_name: &manifest.name,
                        agent_id: agent_id_str.as_str(),
                        event: opencarrier_types::agent::HookEvent::AgentLoopEnd,
                        data: serde_json::json!({
                            "iterations": iteration + 1,
                ...
                for tool_call in &response.tool_calls {
                    ...
                    match &verdict {
                        LoopGuardVerdict::CircuitBreak(msg) => {
                            ...
                            return Err(OpenCarrierError::Internal(msg.clone()));
                        }
                    ...
                    if let Some(hook_reg) = hooks {
                        let ctx = crate::hooks::HookContext {
                            ...
                            event: opencarrier_types::agent::HookEvent::BeforeToolCall,
                            data: serde_json::json!({
                    ...
                    .await
                    {
                        ...
                        Err(_) => {
                            ...
                            opencarrier_types::tool::ToolResult {
                                tool_use_id: tool_call.id.clone(),
                                content: format!(
                    ...
                    if let Some(hook_reg) = hooks {
                        let ctx = crate::hooks::HookContext {
                            ...
                            event: opencarrier_types::agent::HookEvent::AfterToolCall,
                            data: serde_json::json!({
                                "tool_name": &tool_call.name,
    ...
    Err(OpenCarrierError::MaxIterationsExceeded(max_iterations))
}
...
mod tests {
    ...
    async fn test_empty_response_after_tool_use_returns_fallback() {
        let memory = opencarrier_memory::MemorySubstrate::open_in_memory(0.01).unwrap();
        let agent_id = opencarrier_types::agent::AgentId::new();
        let mut session = opencarrier_memory::session::Session {
            id: opencarrier_types::agent::SessionId::new(),
            agent_id,
            messages: Vec::new(),

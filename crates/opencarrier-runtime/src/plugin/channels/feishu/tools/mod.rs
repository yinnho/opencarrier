...
/// Register all Feishu tools with the built-in plugin registry.
pub fn register_feishu_tools(registry: &mut crate::plugin::BuiltinPluginRegistry) {
    for spec in all_tool_specs() {
        registry.register_tool("feishu", move || {
            Box::new(FeishuToolProvider::from_spec(&spec)) as Box<dyn ToolProvider>
        });
    }
}

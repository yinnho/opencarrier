...
/// 2. `~/.opencarrier/agents/` (installed templates)
/// 3. `OPENCARRIER_AGENTS_DIR` env var
pub fn discover_template_dirs() -> Vec<PathBuf> {
    ...
    // Installed templates (respects OPENCARRIER_HOME)
    let of_home = if let Ok(h) = std::env::var("OPENCARRIER_HOME") {
        PathBuf::from(h)
    } else if let Some(home) = dirs::home_dir() {
        home.join(".opencarrier")
    } else {
        std::env::temp_dir().join(".opencarrier")
    };
    {
    ...
    if let Ok(env_dir) = std::env::var("OPENCARRIER_AGENTS_DIR") {

//! MCP (Model Context Protocol) server for OpenCarrier.
//!
//! Exposes running agents as MCP tools over JSON-RPC 2.0 stdio.
//! Each agent becomes a callable tool named `opencarrier_agent_{name}`.
//!
...
impl McpBackend {
    ...
    fn resolve_tool_agent(&self, tool_name: &str) -> Option<String> {
        ...
        let agent_name_underscore = tool_name.strip_prefix("opencarrier_agent_")?;
        for (id, name, _) in &agents {
...
fn create_backend(config: Option<std::path::PathBuf>) -> McpBackend {
    ...
    let kernel = match OpenCarrierKernel::boot(config.as_deref()) {
        Ok(k) => k,
        Err(e) => {
        ...
        "tools/list" => {
            ...
            let tools: Vec<Value> = agents
                ...
                .map(|(_, name, description)| {
                    let tool_name = format!("opencarrier_agent_{}", name.replace('-', "_"));
                    let desc = if description.is_empty() {
                        format!("Send a message to OpenCarrier agent '{name}'")
                    } else {
                        description.clone()
                    };
                    json!({
...
mod tests {
    ...
    fn test_handle_initialize() {
        ...
        assert_eq!(resp["result"]["serverInfo"]["name"], "opencarrier");
    }

...
//!
//! Launched by `opencarrier init` (without `--quick`). Takes over the terminal,
...
// ── Migration stubs (migrate crate removed) ───────────────────────────────────

/// Stub type - migrate crate removed
...
struct State {
    ...
    // Migration (stub - migrate crate removed)
    migration_phase: MigrationPhase,
    migration_choice_list: ListState,
    openclaw_path: Option<PathBuf>,
    openclaw_scan: Option<ScanResult>,
...
impl State {
    ...
    fn build_provider_order(&mut self) {
        ...
        for (i, p) in PROVIDERS.iter().enumerate() {
            let detected = if p.name == "claude-code" {
                opencarrier_runtime::drivers::claude_code::claude_code_available()
            } else {
    ...
    fn is_provider_detected(&self, prov_idx: usize) -> bool {
        ...
        if p.name == "claude-code" {
            return opencarrier_runtime::drivers::claude_code::claude_code_available();
        }
        (!p.env_var.is_empty() && std::env::var(p.env_var).is_ok())
...
pub fn run() -> InitResult {
    ...
    let result = loop {
        ...
        // ── Migration detection disabled (migrate crate removed) ──
        if state.step == Step::Migration && state.migration_phase == MigrationPhase::Detecting {
            // Skip migration entirely - migrate crate removed
            state.advance_to_provider();
        }

        // ── Migration background result polling (disabled) ──
        // Migrate crate removed - no background polling needed
        let _ = migrate_rx; // suppress unused warning
...
// ── Migration step key handler (disabled - migrate crate removed) ────────────

fn handle_migration_key(
    state: &mut State,
    code: KeyCode,
    _migrate_tx: &std::sync::mpsc::Sender<Result<MigrationReport, String>>,
) {
    // Migration disabled - just advance to provider on any key
...
fn save_config(state: &mut State) {
    ...
    let opencarrier_dir = if let Ok(h) = std::env::var("OPENCARRIER_HOME") {
        PathBuf::from(h)
    } else {
        match dirs::home_dir() {
            Some(h) => h.join(".opencarrier"),
            None => {
    ...
    let _ = std::fs::create_dir_all(opencarrier_dir.join("agents"));
    let _ = std::fs::create_dir_all(opencarrier_dir.join("data"));
    crate::restrict_dir_permissions(&opencarrier_dir);
...
/// Check if the `opencarrier-desktop` binary exists next to the current exe.
...
fn draw(f: &mut Frame, area: Rect, state: &mut State) {
    ...
    // Header: "OpenCarrier Init  Step X of 7"
...
fn draw_migration_offer(f: &mut Frame, area: Rect, state: &mut State) {
    ...
    if !scan.agents.is_empty() {
        let names_str = scan.agents.join(", ");
...
fn draw_migration_done(f: &mut Frame, area: Rect, state: &State) {
    ...
    if let Some(ref error) = state.migration_error {
        ...
        // Group imported items by kind (using stub ItemKind)
...
fn draw_routing_pick(f: &mut Frame, area: Rect, state: &mut State, tier: usize) {
        ...
        .enumerate()
    {
        if t == tier {
            ...
            let display = opencarrier_types::truncate_str(short, 14);
            summary_spans.push(Span::styled(
                format!("{name}:{display}"),
                Style::default().fg(*c),
            ));
...
fn draw_complete(f: &mut Frame, area: Rect, state: &mut State) {
    ...
    f.render_widget(
        Paragraph::new(Line::from(vec![Span::styled(
            "  How do you want to use OpenCarrier?",
            Style::default()
                .fg(theme::ACCENT)
                .add_modifier(Modifier::BOLD),
        )])),

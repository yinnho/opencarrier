...
function settingsPage() {
  return {
    ...
    wechatQrSrc: null,
    async startQrLogin() { this.wechatQrCode = null; this.wechatQrRaw = null; this.wechatQrStatus = null; this.wechatQrSrc = null; this.wechatPolling = true; try { var res = await OpenCarrierAPI.get('/api/weixin/qrcode?tenant=default'); if (res.data && res.data.qrcode_img_content) { var raw = res.data.qrcode_img_content; // Handle both base64 data and URL responses from WeChat iLink if (typeof raw === 'string' && (raw.startsWith('http://') || raw.startsWith('https://'))) { this.wechatQrSrc = raw; } else { this.wechatQrCode = raw; } this.wechatQrRaw = res.data.qrcode; this.pollQrStatus(); } else { OpenCarrierToast.error('QR code not available'); this.wechatPolling = false; } } catch(e) { OpenCarrierToast.error('Failed to get QR code: ' + (e.message || e)); this.wechatPolling = false; } },

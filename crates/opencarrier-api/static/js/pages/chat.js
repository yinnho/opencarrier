// OpenCarrier Chat Page — Agent chat with markdown + streaming
'use strict';
...
function chatPage() {
  ...
  return {
    ...
    fetchCommands: function() {
      var self = this;
      OpenCarrierAPI.get('/api/commands').then(function(data) {
      ...
      switch (cmd) {
        ...
        case '/new':
          if (self.currentAgent) {
            OpenCarrierAPI.post('/api/agents/' + self.currentAgent.id + '/session/reset', {}).then(function() {
              self.messages = [];
              OpenCarrierToast.success('Session reset');
            }).catch(function(e) { OpenCarrierToast.error('Reset failed: ' + e.message); });
          }
          break;
        case '/compact':
          if (self.currentAgent) {
            self.messages.push({ id: ++msgId, role: 'system', text: 'Compacting session...', meta: '', tools: [] });
            OpenCarrierAPI.post('/api/agents/' + self.currentAgent.id + '/session/compact', {}).then(function(res) {
              self.messages.push({ id: ++msgId, role: 'system', text: res.message || 'Compaction complete', meta: '', tools: [] });
              self.scrollToBottom();
            }).catch(function(e) { OpenCarrierToast.error('Compaction failed: ' + e.message); });
          }
          break;
        case '/stop':
          if (self.currentAgent) {
            OpenCarrierAPI.post('/api/agents/' + self.currentAgent.id + '/stop', {}).then(function(res) {
              self.messages.push({ id: ++msgId, role: 'system', text: res.message || 'Run cancelled', meta: '', tools: [] });
              self.sending = false;
              self.scrollToBottom();
            }).catch(function(e) { OpenCarrierToast.error('Stop failed: ' + e.message); });
          }
          break;
        case '/usage':
          if (self.currentAgent) {
        ...
        case '/context':
          // Send via WS command
          if (self.currentAgent && OpenCarrierAPI.isWsConnected()) {
            OpenCarrierAPI.wsSend({ type: 'command', command: 'context', args: '' });
          } else {
        ...
        case '/verbose':
          if (self.currentAgent && OpenCarrierAPI.isWsConnected()) {
            OpenCarrierAPI.wsSend({ type: 'command', command: 'verbose', args: cmdArgs });
          } else {
            self.messages.push({ id: ++msgId, role: 'system', text: 'Not connected. Connect to an agent first.', meta: '', tools: [] });
            self.scrollToBottom();
          }
        ...
        case '/queue':
          if (self.currentAgent && OpenCarrierAPI.isWsConnected()) {
            OpenCarrierAPI.wsSend({ type: 'command', command: 'queue', args: '' });
          } else {
          ...
          break;
        case '/status':
          OpenCarrierAPI.get('/api/status').then(function(s) {
    ...
    selectAgent(agent) {
      ...
      if (!localStorage.getItem('of-chat-tips-seen')) {
        ...
        this.messages.push({
          id: ++localMsgId,
          role: 'system',
          text: '**Welcome to OpenCarrier Chat!**\n\n' +
    ...
    async loadSession(agentId) {
      var self = this;
      try {
        var data = await OpenCarrierAPI.get('/api/agents/' + agentId + '/session');
    ...
    async loadSessions(agentId) {
      try {
        var data = await OpenCarrierAPI.get('/api/agents/' + agentId + '/sessions');
    ...
    async createSession() {
      ...
      try {
        await OpenCarrierAPI.post('/api/agents/' + this.currentAgent.id + '/sessions', {
        ...
        this.messages = [];
        this.scrollToBottom();
        if (typeof OpenCarrierToast !== 'undefined') OpenCarrierToast.success('New session created');
      } catch(e) {
        if (typeof OpenCarrierToast !== 'undefined') OpenCarrierToast.error('Failed to create session');
      }
    },
    ...
    async switchSession(sessionId) {
      if (!this.currentAgent) return;
      try {
        await OpenCarrierAPI.post('/api/agents/' + this.currentAgent.id + '/sessions/' + sessionId + '/switch', {});
        this.messages = [];
        await this.loadSession(this.currentAgent.id);
        await this.loadSessions(this.currentAgent.id);
        ...
        if (typeof OpenCarrierToast !== 'undefined') OpenCarrierToast.error('Failed to switch session');
      }
    },
    ...
    connectWs(agentId) {
      ...
      OpenCarrierAPI.wsConnect(agentId, {
        onOpen: function() {
    ...
    async _sendPayload(finalText, uploadedFiles, msgImages) {
      ...
      // HTTP fallback
      if (!OpenCarrierAPI.isWsConnected()) {
        OpenCarrierToast.info('Using HTTP mode (no streaming)');
      }
    ...
    stopAgent: function() {
      if (!this.currentAgent) return;
      var self = this;
      OpenCarrierAPI.post('/api/agents/' + this.currentAgent.id + '/stop', {}).then(function(res) {
    ...
    addFiles(files) {
      ...
      for (var i = 0; i < files.length; i++) {
        ...
        if (file.size > 10 * 1024 * 1024) {
          OpenCarrierToast.warn('File "' + file.name + '" exceeds 10MB limit');
          continue;
        }
        ...
        if (!typeOk) {
          OpenCarrierToast.warn('File type not supported: ' + file.name);
          continue;
        }
        var preview = null;
    ...
    startRecording: async function() {
      ...
      try {
        ...
        if (typeof OpenCarrierToast !== 'undefined') OpenCarrierToast.error('Microphone access denied');
      }
    },
    ...
    _handleRecordingComplete: async function() {
      ...
      try {
        ...
        this.messages = this.messages.filter(function(m) { return !m.thinking || m.role !== 'system'; });
        if (typeof OpenCarrierToast !== 'undefined') OpenCarrierToast.error('Failed to upload audio: ' + (e.message || 'unknown error'));
      }
    },

//! Real HTTP integration tests for the OpenCarrier API.
//!
...
//!
//! Run: cargo test -p opencarrier-api --test api_integration_test -- --nocapture

use axum::Router;
use opencarrier_api::middleware;
use opencarrier_api::routes::{self, AppState};
use opencarrier_api::ws;
use opencarrier_kernel::OpenCarrierKernel;
use opencarrier_types::config::{DefaultModelConfig, KernelConfig};
use std::sync::Arc;
...
async fn test_status_endpoint() {
    ...
    assert_eq!(body["agent_count"], 0); // no agents auto-spawned
...
async fn test_spawn_list_kill_agent() {
    ...
    assert_eq!(test_agent["model_provider"], "chat");
...
"#
    ...
    // List should show 2 remaining agents
    let resp = client
        .get(format!("{}/api/agents", server.base_url))
        .send()
        .await

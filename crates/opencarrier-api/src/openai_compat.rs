...
//!
//! Allows any OpenAI-compatible client library to talk to OpenCarrier agents.
//! The `model` field resolves to an agent (by name, UUID, or `opencarrier:<name>`),
...
use opencarrier_types::message::{ContentBlock, Message, MessageContent, Role, StopReason};
...
fn resolve_agent(state: &AppState, model: &str) -> Option<(AgentId, String)> {
    // 1. "opencarrier:<name>" → find agent by name
    if let Some(name) = model.strip_prefix("opencarrier:") {
...
pub async fn list_models(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let models: Vec<ModelObject> = agents
        .iter()
        .map(|e| ModelObject {
            id: format!("opencarrier:{}", e.name),
            object: "model",
            created,
            owned_by: "opencarrier".to_string(),
        })
        .collect();

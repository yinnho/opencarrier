//! Channel bridge wiring — connects the OpenCarrier kernel to channel adapters.
//!
//! Implements `ChannelBridgeHandle` on `OpenCarrierKernel` and provides the
//! `start_channel_bridge()` entry point called by the daemon.

use opencarrier_channels::bridge::{BridgeManager, ChannelBridgeHandle};
use opencarrier_channels::discord::DiscordAdapter;
use opencarrier_channels::email::EmailAdapter;
use opencarrier_channels::google_chat::GoogleChatAdapter;
use opencarrier_channels::irc::IrcAdapter;
use opencarrier_channels::matrix::MatrixAdapter;
use opencarrier_channels::mattermost::MattermostAdapter;
use opencarrier_channels::rocketchat::RocketChatAdapter;
use opencarrier_channels::router::AgentRouter;
use opencarrier_channels::signal::SignalAdapter;
use opencarrier_channels::slack::SlackAdapter;
use opencarrier_channels::teams::TeamsAdapter;
use opencarrier_channels::telegram::TelegramAdapter;
use opencarrier_channels::twitch::TwitchAdapter;
use opencarrier_channels::types::ChannelAdapter;
use opencarrier_channels::whatsapp::WhatsAppAdapter;
use opencarrier_channels::xmpp::XmppAdapter;
use opencarrier_channels::zulip::ZulipAdapter;
// Wave 3
use opencarrier_channels::bluesky::BlueskyAdapter;
use opencarrier_channels::feishu::FeishuAdapter;
use opencarrier_channels::line::LineAdapter;
use opencarrier_channels::mastodon::MastodonAdapter;
use opencarrier_channels::messenger::MessengerAdapter;
use opencarrier_channels::reddit::RedditAdapter;
use opencarrier_channels::revolt::RevoltAdapter;
use opencarrier_channels::viber::ViberAdapter;
// Wave 4
use opencarrier_channels::flock::FlockAdapter;
use opencarrier_channels::guilded::GuildedAdapter;
use opencarrier_channels::keybase::KeybaseAdapter;
use opencarrier_channels::nextcloud::NextcloudAdapter;
use opencarrier_channels::nostr::NostrAdapter;
use opencarrier_channels::pumble::PumbleAdapter;
use opencarrier_channels::threema::ThreemaAdapter;
use opencarrier_channels::twist::TwistAdapter;
use opencarrier_channels::webex::WebexAdapter;
// Wave 5
use async_trait::async_trait;
use opencarrier_channels::dingtalk::DingTalkAdapter;
use opencarrier_channels::dingtalk_stream::DingTalkStreamAdapter;
use opencarrier_channels::discourse::DiscourseAdapter;
use opencarrier_channels::gitter::GitterAdapter;
use opencarrier_channels::gotify::GotifyAdapter;
use opencarrier_channels::linkedin::LinkedInAdapter;
use opencarrier_channels::mumble::MumbleAdapter;
use opencarrier_channels::ntfy::NtfyAdapter;
use opencarrier_channels::webhook::WebhookAdapter;
use opencarrier_channels::wecom::WeComAdapter;
...
use opencarrier_runtime::str_utils::safe_truncate_str;

/// Wraps `OpenCarrierKernel` to implement `ChannelBridgeHandle`.
...
impl ChannelBridgeHandle for KernelBridgeAdapter {
    ...
    async fn spawn_agent_by_name(&self, manifest_name: &str) -> Result<AgentId, String> {
        // Look for manifest at ~/.opencarrier/agents/{name}/agent.toml
    ...
    async fn uptime_info(&self) -> String {
        ...
        if hours > 0 {
            format!(
                "OpenCarrier status: {}h {}m uptime, {} agent(s)",
                hours,
                mins,
                agents.len()
            )
            ...
            format!(
                "OpenCarrier status: {}m uptime, {} agent(s)",
                mins,
                agents.len()
            )
        }
    ...
    async fn list_skills_text(&self) -> String {
        ...
        if skills.is_empty() {
            return "No skills installed. Place skills in ~/.opencarrier/skills/ or install from the marketplace.".to_string();
        }
        ...
        let result = self
            ...
            .execute_run(
                ...
                |step_agent| match step_agent {
                    opencarrier_kernel::workflow::StepAgent::ById { id } => {
                        let aid: AgentId = id.parse().ok()?;
                        let entry = registry_ref.get(aid)?;
                        Some((aid, entry.name.clone()))
                    }
                    opencarrier_kernel::workflow::StepAgent::ByName { name } => {
    ...
    async fn list_schedules_text(&self) -> String {
        ...
        for job in &jobs {
            ...
            let sched = match &job.schedule {
                opencarrier_types::scheduler::CronSchedule::Cron { expr, .. } => expr.clone(),
                opencarrier_types::scheduler::CronSchedule::Every { every_secs } => {
                    format!("every {every_secs}s")
                }
                opencarrier_types::scheduler::CronSchedule::At { at } => {
    ...
    async fn channel_overrides(
        &self,
        channel_type: &str,
    ) -> Option<opencarrier_types::config::ChannelOverrides> {
    ...
    async fn record_delivery(
        ...
        let receipt = if success {
            opencarrier_kernel::DeliveryTracker::sent_receipt(channel, recipient)
        } else {
            opencarrier_kernel::DeliveryTracker::failed_receipt(
                channel,
                recipient,
    ...
    async fn peers_text(&self) -> String {
        ...
        "P2P networking disabled (wire crate removed)".to_string()
    }
    ...
    async fn a2a_agents_text(&self) -> String {
        ...
        for (url, card) in agents.iter() {
            ...
            if !desc.is_empty() {
                let short = opencarrier_types::truncate_str(desc, 60);
...
fn parse_trigger_pattern(s: &str) -> Option<opencarrier_kernel::triggers::TriggerPattern> {
    use opencarrier_kernel::triggers::TriggerPattern;
...
pub async fn start_channel_bridge(kernel: Arc<OpenCarrierKernel>) -> Option<BridgeManager> {
...
pub async fn reload_channels_from_disk(
    ...
    let fresh_config = opencarrier_kernel::config::load_config(Some(&config_path));

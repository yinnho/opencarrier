...
/// Result of converting a SKILL.md into OpenCarrier format.
#[derive(Debug, Clone)]
pub struct ConvertedSkillMd {
    ...
    /// Tool name translations applied (openclaw_name → opencarrier_name).
...
/// Full conversion of a SKILL.md directory to OpenCarrier format.
///
...
pub fn convert_skillmd(dir: &Path) -> Result<ConvertedSkillMd, SkillError> {
    ...
    if let Some(ref meta) = frontmatter.metadata.openclaw {
        ...
        // Convert commands to OpenCarrier tool definitions
        for cmd in &meta.commands {
            ...
            let opencarrier_name = if let Some(mapped) = tool_compat::map_tool_name(&cmd.name) {
                tool_translations.push((cmd.name.clone(), mapped.to_string()));
                mapped.to_string()
            } else if tool_compat::is_known_opencarrier_tool(&cmd.name) {
                cmd.name.clone()
            } else {
...
/// Convert an in-memory SKILL.md string into OpenCarrier format.
///
...
/// Convert an OpenClaw Node.js skill directory into an OpenCarrier SkillManifest.
///
...
/// Write an OpenCarrier skill.toml manifest for an OpenClaw skill.
pub fn write_opencarrier_manifest(dir: &Path, manifest: &SkillManifest) -> Result<(), SkillError> {

...
/// Normalize a tool name to its canonical OpenCarrier form.
///
/// If the name is already a known OpenCarrier tool, returns it as-is.
...
pub fn normalize_tool_name(name: &str) -> &str {
    if is_known_opencarrier_tool(name) {
        return name;
    }
    map_tool_name(name).unwrap_or(name)
}

/// Check if a tool name is a known OpenCarrier built-in tool.
pub fn is_known_opencarrier_tool(name: &str) -> bool {
    matches!(
        name,
        "file_read"
            | "file_write"
...
mod tests {
    ...
    fn test_normalize_tool_name() {
        // Known OpenCarrier tools pass through unchanged
    ...
    #[test]
    fn test_is_known_opencarrier_tool() {
        ...
        // Unknown
        assert!(!is_known_opencarrier_tool("unknown"));
        assert!(!is_known_opencarrier_tool("Read"));
        assert!(!is_known_opencarrier_tool("Bash"));
    }
}

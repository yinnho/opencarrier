...
pub struct MatrixConfig {
    ...
    /// Bot user ID (e.g., "@opencarrier:matrix.org").
    pub user_id: String,
...
pub struct IrcConfig {
    ...
    /// Channels to join (e.g., `["#opencarrier", "#general"]`).
    #[serde(default, deserialize_with = "deserialize_string_or_int_vec")]
...
impl Default for MumbleConfig {
    fn default() -> Self {
        Self {
            host: String::new(),
            port: 64738,
            username: "opencarrier".to_string(),
...
mod tests {
    ...
    fn test_validate_missing_env_vars() {
        ...
        config.channels.discord = Some(DiscordConfig {
            bot_token_env: "OPENCARRIER_TEST_NONEXISTENT_VAR_DC".to_string(),
            ..Default::default()
        });
        let warnings = config.validate();
    ...
    fn test_irc_config_defaults() {
        ...
        assert_eq!(irc.nick, "opencarrier");
        assert!(!irc.use_tls);
    }

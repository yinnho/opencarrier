...
fn build_compose_yaml(manifest: &McpServerManifest, install: &McpDockerInstall) -> String {
    let container_name = if install.container_name.is_empty() {
        format!("opencarrier-mcp-{}", manifest.name)
    } else {
        install.container_name.clone()
    };
...
"#,
        service_name = manifest.name.replace('-', "_"),
        image = install.image,
        container_name = container_name,
        restart_policy = install.restart_policy,

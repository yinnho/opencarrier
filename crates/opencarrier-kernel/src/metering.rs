...
use opencarrier_memory::usage::{ModelUsage, UsageRecord, UsageStore, UsageSummary};
use opencarrier_types::agent::{AgentId, ResourceQuota};
...
impl MeteringEngine {
    ...
    pub fn check_quota(&self, agent_id: AgentId, quota: &ResourceQuota) -> OpenCarrierResult<()> {
        // Hourly check
        if quota.max_cost_per_hour_usd > 0.0 {
            ...
            if hourly_cost >= quota.max_cost_per_hour_usd {
                return Err(OpenCarrierError::QuotaExceeded(format!(
    ...
    pub fn budget_status(&self, budget: &opencarrier_types::config::BudgetConfig) -> BudgetStatus {
    ...
    pub fn get_by_model(&self) -> OpenCarrierResult<Vec<ModelUsage>> {
    ...
    pub fn estimate_cost_with_catalog(
        catalog: &opencarrier_runtime::model_catalog::ModelCatalog,
        model: &str,
        input_tokens: u64,
        output_tokens: u64,
    ) -> f64 {

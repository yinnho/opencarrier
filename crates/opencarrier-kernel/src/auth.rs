...
//! to OpenCarrier users with roles, then enforces permission checks on actions.

use dashmap::DashMap;
use opencarrier_types::agent::UserId;
use opencarrier_types::config::UserConfig;
...
pub struct AuthManager {
    /// Known users by their OpenCarrier user ID.
...
impl AuthManager {
    ...
    ///
    /// Returns the OpenCarrier UserId if a matching channel binding exists,
    ...
    pub fn authorize(&self, user_id: UserId, action: &Action) -> OpenCarrierResult<()> {
        let identity = self
            .users
            .get(&user_id)
            .ok_or_else(|| OpenCarrierError::AuthDenied("Unknown user".to_string()))?;

        let required = action.required_role();
        if identity.role >= required {
            Ok(())
        } else {
            Err(OpenCarrierError::AuthDenied(format!(

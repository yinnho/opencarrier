...
//!
//! Embeds the gateway JS at compile time, extracts it to `~/.opencarrier/whatsapp-gateway/`,
//! runs `npm install` if needed, and spawns `node index.js` as a managed child process
//! that auto-restarts on crash.

use crate::config::opencarrier_home;
...
fn gateway_dir() -> PathBuf {
    opencarrier_home().join("whatsapp-gateway")
}
...
pub async fn start_whatsapp_gateway(kernel: &Arc<super::kernel::OpenCarrierKernel>) {
    ...
    let opencarrier_url = format!("http://{api_listen}");
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let child = tokio::process::Command::new(node_cmd)
                ...
                .env("OPENCARRIER_URL", &opencarrier_url)
                .env("OPENCARRIER_DEFAULT_AGENT", &default_agent)
...
mod tests {
    ...
    fn test_embedded_files_not_empty() {
        ...
        assert!(GATEWAY_PACKAGE_JSON.contains("@opencarrier/whatsapp-gateway"));
    }
    ...
    #[test]
    fn test_gateway_dir_under_opencarrier_home() {
        let dir = gateway_dir();
        ...
        assert!(dir
            ...
            .unwrap()
            .to_string_lossy()
            .contains(".opencarrier"));
    }

    #[test]
    fn test_write_if_changed_creates_new_file() {
        let tmp = std::env::temp_dir().join("opencarrier_test_gateway");

...
//!
//! Run with: GROQ_API_KEY=gsk_... cargo test -p opencarrier-kernel --test multi_agent_test -- --nocapture
...
fn test_config() -> KernelConfig {
    let tmp = std::env::temp_dir().join("opencarrier-multi-agent-test");
...
async fn test_six_agent_fleet() {
    ...
    let kernel = OpenCarrierKernel::boot_with_config(test_config()).expect("Kernel should boot");

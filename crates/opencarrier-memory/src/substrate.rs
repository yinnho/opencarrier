...
impl MemorySubstrate {
    ...
    pub fn load_paired_devices(&self) -> OpenCarrierResult<Vec<serde_json::Value>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| OpenCarrierError::Memory(e.to_string()))?;
        ...
        for row in rows {
            devices.push(row.map_err(|e| OpenCarrierError::Memory(e.to_string()))?);
        }
        Ok(devices)
    }
    ...
    pub fn remove_paired_device(&self, device_id: &str) -> OpenCarrierResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| OpenCarrierError::Memory(e.to_string()))?;

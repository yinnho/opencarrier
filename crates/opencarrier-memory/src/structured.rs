...
impl StructuredStore {
    ...
    pub fn list_kv(
        ...
        for row in rows {
            let (key, blob) = row.map_err(|e| OpenCarrierError::Memory(e.to_string()))?;
        ...
        match result {
            Ok((name, manifest_blob, state_str, created_str, session_id_str, identity_str)) => {
                ...
                let session_id = session_id_str
                    ...
                    .map(opencarrier_types::agent::SessionId)
                    .unwrap_or_else(opencarrier_types::agent::SessionId::new);
    ...
    pub fn load_all_agents(&self) -> OpenCarrierResult<Vec<AgentEntry>> {
        ...
        for row in rows {
            ...
            let manifest: opencarrier_types::agent::AgentManifest = match rmp_serde::from_slice(
                &manifest_blob,
            ) {
                Ok(m) => m,
                Err(e) => {

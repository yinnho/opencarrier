...
use opencarrier_types::memory::{MemoryFilter, MemoryFragment, MemoryId, MemorySource};
...
impl SemanticStore {
    ...
    pub fn recall_with_embedding(
        ...
        for row_result in rows {
            let (
                ...
                access_count,
                embedding_bytes,
            ) = row_result.map_err(|e| OpenCarrierError::Memory(e.to_string()))?;
    ...
    pub fn forget(&self, id: MemoryId) -> OpenCarrierResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| OpenCarrierError::Internal(e.to_string()))?;

...
impl ChannelAdapter for NostrAdapter {
    ...
    ) -> Result<Pin<Box<dyn Stream<Item = ChannelMessage> + Send>>, Box<dyn std::error::Error>>
    {
        ...
        info!(
            ...
            opencarrier_types::truncate_str(&pubkey, 16)
        );
                    ...
                    let should_reconnect = loop {
                        let msg = tokio::select! {
                            _ = relay_shutdown_rx.changed() => {
                                ...
                                // Send CLOSE
                                let close_msg = serde_json::json!(["CLOSE", "opencarrier-sub"]);
                                let _ = write.send(
                        ...
                        let channel_msg = ChannelMessage {
                            ...
                            sender: ChannelUser {
                                ...
                                display_name: format!(
                                    "{}...",
                                    opencarrier_types::truncate_str(&sender_pubkey, 8)
                                ),
...
mod tests {
    ...
    fn test_nostr_build_subscription() {
        ...
        assert!(sub.contains("opencarrier-sub"));

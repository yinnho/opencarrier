...
pub struct ChannelUser {
    ...
    /// Optional mapping to an OpenCarrier user identity.
    pub opencarrier_user: Option<String>,
}
...
// Re-export policy/format types from opencarrier-types for convenience.
pub use opencarrier_types::config::{DmPolicy, GroupPolicy, OutputFormat};

/// Trait that every channel adapter must implement.
///
/// A channel adapter bridges a messaging platform to the OpenCarrier kernel by converting
...
pub fn split_message(text: &str, max_len: usize) -> Vec<&str> {
    ...
    while !remaining.is_empty() {
        ...
        let safe_end = opencarrier_types::truncate_str(remaining, max_len).len();

//! IRC channel adapter for the OpenCarrier channel bridge.
//!
...
pub struct IrcAdapter {
    ...
    /// IRC channels to join (e.g., ["#opencarrier", "#bots"]).
    channels: Vec<String>,
        ...
        *self.write_tx.write().await = Some(write_cmd_tx.clone());
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                registration.push_str(&format!("USER {nick} 0 * :OpenCarrier Bot\r\n"));
                    ...
                    tokio::select! {
                        ...
                        _ = shutdown_rx.changed() => {
                            if *shutdown_rx.borrow() {
                                ...
                                let _ = writer.write_all(b"QUIT :OpenCarrier shutting down\r\n").await;
                                return;
                            }
                        }
                    }
...
mod tests {
    ...
    fn test_irc_adapter_creation() {
        let adapter = IrcAdapter::new(
            ...
            6667,
            "opencarrier".to_string(),
            None,
            vec!["#opencarrier".to_string()],
            false,
        );
    ...
    fn test_parse_irc_line_no_trailing() {
        let line = parse_irc_line(":alice!alice@host JOIN #opencarrier").unwrap();
    ...
    fn test_parse_privmsg_channel() {
        let line = IrcLine {
            ...
            params: vec!["#opencarrier".to_string()],
            trailing: Some("Hello from IRC!".to_string()),
        };

        let msg = parse_privmsg(&line, "opencarrier-bot").unwrap();
        assert_eq!(msg.channel, ChannelType::Custom("irc".to_string()));
        assert_eq!(msg.sender.display_name, "alice");
        assert_eq!(msg.sender.platform_id, "#opencarrier");
        assert!(msg.is_group);
    ...
    fn test_parse_privmsg_dm() {
        let line = IrcLine {
            ...
            command: "PRIVMSG".to_string(),
            params: vec!["opencarrier-bot".to_string()],
    ...
    fn test_parse_privmsg_skips_self() {
        let line = IrcLine {
            prefix: Some("opencarrier-bot!bot@host".to_string()),
            command: "PRIVMSG".to_string(),
            params: vec!["#opencarrier".to_string()],
            trailing: Some("My own message".to_string()),
        };

        let msg = parse_privmsg(&line, "opencarrier-bot");
        assert!(msg.is_none());
    }

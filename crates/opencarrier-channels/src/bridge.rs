//! Channel bridge — connects channel adapters to the OpenCarrier kernel.
//!
//! Defines `ChannelBridgeHandle` (implemented by opencarrier-api on the kernel) and
...
use opencarrier_types::config::{ChannelOverrides, DmPolicy, GroupPolicy, OutputFormat};
use opencarrier_types::message::ContentBlock;
use std::sync::Arc;
...
///
/// Defined here to avoid circular deps (opencarrier-channels can't depend on opencarrier-kernel).
/// Implemented in opencarrier-api on the actual kernel.
#[async_trait]
...
async fn dispatch_message(
    ...
    if router.has_broadcast(&message.sender.platform_id) {
        ...
        if !targets.is_empty() {
            ...
            match strategy {
                opencarrier_types::config::BroadcastStrategy::Parallel => {
                ...
                opencarrier_types::config::BroadcastStrategy::Sequential => {
                    for (name, maybe_id) in &targets {
                        if let Some(aid) = maybe_id {
    ...
    let agent_id = router.resolve(
        ...
        message.sender.opencarrier_user.as_deref(),
    );
...
async fn handle_command(
    ...
    match name {
        "start" => {
            ...
            let mut msg = "Welcome to OpenCarrier! I connect you to AI agents.\n\nAvailable agents:\n"
                .to_string();
            if agents.is_empty() {
        ...
        "new" => {
            ...
            let agent_id = router.resolve(
                ...
                &sender.platform_id,
                sender.opencarrier_user.as_deref(),
            );
            match agent_id {
                Some(aid) => handle
                    .reset_session(aid)

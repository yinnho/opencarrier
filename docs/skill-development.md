...
Skills are pluggable tool bundles that extend agent capabilities in OpenCarrier. A skill packages one or more tools with their implementation, letting agents do things that built-in tools do not cover. This guide covers skill creation, the manifest format, Python and WASM runtimes, publishing to FangHub, and CLI management.
...
Skills are installed to `~/.opencarrier/skills/` and made available to agents through the skill registry. OpenCarrier ships with **60 bundled skills** that are compiled into the binary and available immediately.
...
OpenCarrier includes 60 expert knowledge skills compiled into the binary (no installation needed):
...
1. OpenCarrier sends a JSON payload to the script's stdin:
...
```python
#!/usr/bin/env python3
"""OpenCarrier skill: web-summarizer"""
import json
import sys
...
def summarize_url(url: str) -> str:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "OpenCarrier-Skill/1.0"})
...
### Using the OpenCarrier Python SDK

For more advanced skills, use the Python SDK (`sdk/python/opencarrier_sdk.py`):

```python
#!/usr/bin/env python3
from opencarrier_sdk import SkillHandler
...
```bash
opencarrier skill install /path/to/my-skill
```

This reads the `skill.toml`, validates the manifest, and copies the skill to `~/.opencarrier/skills/my-skill/`.

### From FangHub

```bash
opencarrier skill install web-summarizer
```
...
```bash
opencarrier skill install https://github.com/user/opencarrier-skill-example.git
```
...
```bash
opencarrier skill remove web-summarizer
```
...
FangHub is the community skill marketplace for OpenCarrier.
...
```bash
opencarrier skill search "web scraping"
```
...
OpenCarrier can install and run OpenClaw-format skills. The skill installer auto-detects OpenClaw skills (by looking for `package.json` + `index.ts`/`index.js`) and converts them.

### Automatic Conversion

```bash
opencarrier skill install /path/to/openclaw-skill
```

If the directory contains an OpenClaw-style skill (Node.js package), OpenCarrier:

1. Detects the OpenClaw format.
2. Generates a `skill.toml` manifest from `package.json`.
3. Maps tool names to OpenCarrier conventions.
4. Copies the skill to the OpenCarrier skills directory.
...
```bash
opencarrier skill install /path/to/skill-directory
```

Skills imported via `opencarrier migrate --from openclaw` are also scanned and reported in the migration report, with instructions for manual reinstallation.

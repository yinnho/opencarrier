# OpenCarrier Security Architecture
...
system in the OpenCarrier Agent Operating System.  All struct names, function
...
OpenCarrier implements **defense-in-depth** security.  No single mechanism is
...
|---|--------|-------|------------------|
| 1 | Capability-Based Security | `opencarrier-types` | Unauthorized actions by agents |
| 2 | WASM Dual Metering | `opencarrier-runtime` | Infinite loops, CPU DoS |
| 3 | Merkle Audit Trail | `opencarrier-runtime` | Tampered audit logs |
| 4 | Taint Tracking | `opencarrier-types` | Prompt injection, data exfiltration |
| 5 | Ed25519 Manifest Signing | `opencarrier-types` | Supply chain attacks |
| 6 | SSRF Protection | `opencarrier-runtime` | Server-Side Request Forgery |
| 7 | Secret Zeroization | `opencarrier-runtime`, `opencarrier-channels` | Memory forensics, key leakage |
| 8 | OFP Mutual Auth | `opencarrier-wire` | Unauthorized peer connections |
| 9 | Security Headers | `opencarrier-api` | XSS, clickjacking, MIME sniffing |
| 10 | GCRA Rate Limiter | `opencarrier-api` | API abuse, denial of service |
| 11 | Path Traversal Prevention | `opencarrier-runtime` | Directory traversal attacks |
| 12 | Subprocess Sandbox | `opencarrier-runtime` | Secret leakage via child processes |
| 13 | Prompt Injection Scanner | `opencarrier-skills` | Malicious skill prompts |
| 14 | Loop Guard | `opencarrier-runtime` | Stuck agent tool loops |
| 15 | Session Repair | `opencarrier-runtime` | Corrupted LLM conversation history |
| 16 | Health Endpoint Redaction | `opencarrier-api` | Information leakage |

---

## 2. Capability-Based Security

**Source:** `opencarrier-types/src/capability.rs`

OpenCarrier uses capability-based security.  An agent can only perform actions
...
**Source:** `opencarrier-runtime/src/sandbox.rs`
...
**Source:** `opencarrier-runtime/src/audit.rs`
...
**Source:** `opencarrier-types/src/taint.rs`

OpenCarrier implements a lattice-based taint propagation model that prevents
...
**Source:** `opencarrier-types/src/manifest_signing.rs`
...
**Source:** `opencarrier-runtime/src/host_functions.rs`
...
OpenCarrier uses `Zeroizing<String>` from the `zeroize` crate on every field
...
**LLM Drivers** (`opencarrier-runtime/src/drivers/`):
...
**Channel Adapters** (`opencarrier-channels/src/`):
...
**Web Search** (`opencarrier-runtime/src/web_search.rs`):
...
**Embedding** (`opencarrier-runtime/src/embedding.rs`):
...
**Source:** `opencarrier-wire/src/peer.rs`

The OpenCarrier Wire Protocol (OFP) uses HMAC-SHA256 with nonce-based mutual
...
**Source:** `opencarrier-api/src/middleware.rs`
...
**Source:** `opencarrier-api/src/rate_limiter.rs`

OpenCarrier uses the Generic Cell Rate Algorithm (GCRA) for cost-aware API
...
**Source:** `opencarrier-runtime/src/subprocess_sandbox.rs`
...
**Source:** `opencarrier-skills/src/verify.rs`
...
**Source:** `opencarrier-runtime/src/loop_guard.rs`
...
**Source:** `opencarrier-runtime/src/session_repair.rs`
...
**Source:** `opencarrier-api/src/routes.rs`

OpenCarrier provides two health endpoints with different information levels.

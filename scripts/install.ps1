# OpenCarrier installer for Windows
# Usage: iwr -useb https://opencarrier.sh/install.ps1 | iex
...
function Write-Banner {
    Write-Host ""
    Write-Host "  OpenCarrier Installer" -ForegroundColor Cyan
...
function Get-Architecture {
    ...
    switch ($archUpper) {
        ...
        default {
            ...
            Write-Host "  Try: cargo install --git https://github.com/RightNow-AI/opencarrier opencarrier-cli" -ForegroundColor Yellow
            exit 1
        }
    }
}
...
function Get-LatestVersion {
    ...
    try {
        ...
        Write-Host "    cargo install --git https://github.com/$Repo opencarrier-cli"
        exit 1
    }
}

function Install-OpenCarrier {
    Write-Banner

    $arch = Get-Architecture
    $version = Get-LatestVersion
    $target = "${arch}-pc-windows-msvc"
    $archive = "opencarrier-${target}.zip"
    $url = "https://github.com/$Repo/releases/download/$version/$archive"
    $checksumUrl = "$url.sha256"

    Write-Host "  Installing OpenCarrier $version for $target..."
    ...
    # Download to temp
    $tempDir = Join-Path ([System.IO.Path]::GetTempPath()) "opencarrier-install"
    ...
    $exePath = Join-Path $tempDir "opencarrier.exe"
    if (-not (Test-Path $exePath)) {
        # May be nested in a directory
        $found = Get-ChildItem -Path $tempDir -Filter "opencarrier.exe" -Recurse | Select-Object -First 1
        if ($found) {
            $exePath = $found.FullName
        }
        else {
            Write-Host "  Could not find opencarrier.exe in archive." -ForegroundColor Red
            Remove-Item -Recurse -Force $tempDir -ErrorAction SilentlyContinue
            exit 1
        }
    }
    ...
    # Install
    Copy-Item -Path $exePath -Destination (Join-Path $InstallDir "opencarrier.exe") -Force
    ...
    # Verify
    $installedExe = Join-Path $InstallDir "opencarrier.exe"
    if (Test-Path $installedExe) {
        try {
            $versionOutput = & $installedExe --version 2>&1
            Write-Host ""
            Write-Host "  OpenCarrier installed successfully! ($versionOutput)" -ForegroundColor Green
        }
        catch {
            Write-Host ""
            Write-Host "  OpenCarrier binary installed to $installedExe" -ForegroundColor Green
        }
    }

    Write-Host ""
    Write-Host "  Get started:" -ForegroundColor Cyan
    Write-Host "    opencarrier init"
    Write-Host ""

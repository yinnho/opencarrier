...
#   docker build -f scripts/docker/install-smoke.Dockerfile \
#     --build-arg OPENCARRIER_SMOKE_FULL=1 .
...
ARG OPENCARRIER_SMOKE_FULL=0
RUN if [ "$OPENCARRIER_SMOKE_FULL" = "1" ]; then \
        bash /tmp/install.sh; \
    else \
        # 1. Syntax check
...
RUN if [ "$OPENCARRIER_SMOKE_FULL" = "1" ] && [ -f "$HOME/.opencarrier/bin/opencarrier" ]; then \
        $HOME/.opencarrier/bin/opencarrier --version && \
        echo "PASS: opencarrier binary works"; \
    else \

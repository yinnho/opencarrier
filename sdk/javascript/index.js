/**
 * @opencarrier/sdk — Official JavaScript client for the OpenCarrier Agent OS REST API.
 *
 * Usage:
 *   const { OpenCarrier } = require("@opencarrier/sdk");
...
class OpenCarrierError extends Error {
  constructor(message, status, body) {
    super(message);
    this.name = "OpenCarrierError";
    this.status = status;
    this.body = body;
  }
}
...
class OpenCarrier {
  /**
   * @param {string} baseUrl - OpenCarrier server URL (e.g. "http://localhost:3000")
   * @param {object} [opts]
  ...
  async _request(method, path, body) {
    ...
    if (!res.ok) {
      ...
      throw new OpenCarrierError("HTTP " + res.status + ": " + text, res.status, text);
    }
...
class AgentResource {
  ...
  async upload(id, file, filename) {
    ...
    if (!res.ok) throw new OpenCarrierError("Upload failed: " + res.status, res.status);
    return res.json();
  }
...
module.exports = { OpenCarrier: OpenCarrier, OpenCarrierError: OpenCarrierError };
